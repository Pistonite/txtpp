...
impl TagState {
    ...
    pub fn create(&mut self, tag: &str) -> Result<(), TagStateError> {
        ...
        for k in self.stored.keys() {
            if k.starts_with(tag) || tag.starts_with(k) {
                if k == tag {
                    return Err(Report::new(TagStateError).attach_printable(format!(
                        "Cannot create new tag `{tag}` because a tag exists with the same name."
                    )));
                }
                return Err(Report::new(TagStateError).attach_printable(format!(
    ...
    pub fn inject_tags(&mut self, output: &str, line_ending: &str) -> String {
        // argument should never have newline
    ...
    pub fn has_tags(&self) -> bool {
        self.listening.is_some() || !self.stored.is_empty()
    }
}
...
impl Display for TagState {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(listening) = &self.listening {
            tags.push(listening.clone());
        }
        tags.extend(self.stored.keys().cloned());
        write!(f, "{}", tags.join(" ,"))
    }
}
#[cfg(test)]
mod tests {
    ...
    fn test_tag_inject_one_prefixed() {
        ...
        assert_eq!("hellocontent1", &output);
        assert_eq!(2, tag_state.stored.len());
    }
    ...
    fn test_tag_inject_one_postfixed() {
        ...
        assert_eq!("content1hello", &output);
        assert_eq!(2, tag_state.stored.len());
    }
    ...
    fn test_tag_inject_one_sandwiched() {
        ...
        assert_eq!("worldcontent1hello", &output);
        assert_eq!(2, tag_state.stored.len());
    }
    ...
    fn test_tag_inject_many() {
        ...
        assert_eq!("content1content2content3", &output);
    ...
    fn test_tag_inject_none() {
        ...
        assert_eq!("asdftag 1fg   tag 2tag 3asdf", &output);
    ...
    fn test_tag_inject_many_complex() {
        ...
        assert_eq!("asdftag 1fg   content2content3asdf", &output);
